...
namespace euterpe {
    ...
    bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
        ...
        std::unordered_set<std::string> note;
        for(auto i = result.begin(); i!=result.end(); ++i){
            // std::cout << (*i)->toString() << std::endl;
            if(note.count((*i)->toString())==0){
                // std::cout << "no erase" << std::endl;
                note.insert((*i)->toString());
            }else{
...
//        std::set<Address::ptr> temp(result.begin(),result.end());
//        std::vector<Address::ptr> temp2(temp.begin(),temp.end());
//        result = temp2;
        return !result.empty();
    }
    ...
    bool Address::GetInterfaceAddresses(std::multimap<std::string
        ...
        try {
            ...
            EUTERPE_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
        ...
        if(error) {
            EUTERPE_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                                      << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
        ...
        if(result <= 0) {
            EUTERPE_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    /**
     * @brief 用位运算将地址输出 e.g. 192.0.0.1:8080
     * @param os
     * @return
     */
    ...
    IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
        ...
        if(result <= 0) {
            EUTERPE_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                                      << port << ") rt=" << result << " errno=" << errno
                                      << " errstr=" << strerror(errno);
            return nullptr;
        }
